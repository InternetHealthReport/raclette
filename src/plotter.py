...
class Plotter(object):
    ...
    def metric_over_time(self, startpoint, endpoint, metric="median", filename="fig/{}_{}_{}_expid{}_diffrtt_time.pdf", expid=1, tz="UTC", ylim=None, probe_geoloc=None, group = True, label=None, startpoint_label=None, endpoint_label=None):
        ...
        for conn in self.conn:
            all_df.append(pd.read_sql_query("SELECT ts, startpoint, endpoint, median, confhigh, conflow, nbtracks FROM diffrtt where expid=? and startpoint like ? and endpoint like ? order by ts" , conn, "ts", params=(expid, startpoint, endpoint), parse_dates=["ts"]) )
