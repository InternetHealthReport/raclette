...
class SQLiteSaver(multiprocessing.Process):
    ...
    def createdb(self):
        ...
        self.cursor.execute("CREATE TABLE IF NOT EXISTS diffrtt (ts integer, startpoint text, endpoint text, median real, confhigh real, conflow real, nbsamples integer, nbprobes interger, entropy real, expid integer, foreign key(expid) references experiment(id))")
    ...
    def save(self, elem):
        ...
        if not self.expid:
            ...
            ts, startpoint, endpoint, median, high, low, nb_samples, nb_probes, entropy  = data
